# -*- coding: utf-8 -*-
"""Politifact Dataset LSTM and BERT.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1RkMei9p30wJlp1SH03brIQUR0p_PEhKI
"""
...
with open('json_data.json', 'r') as myfile:
    data=myfile.read()

# parse file
obj = json.loads(data)

# columns = ['id', 'mugshot', 'source', 'text', 'edition', 'date', 'rating', 'reason', 'affiliation']
# print(df)
...
true_articles = []
mostly_true_articles = []
half_true_articles = []
mostly_false_articles = []
false_articles = []
pants_on_fire_articles = []

for article in obj:
  if article['rating'] == "false":
    false_articles.append(article)
  elif article['rating'] == "true":
    true_articles.append(article)
  elif article['rating'] == "mostly true":
    mostly_true_articles.append(article)
  elif article['rating'] == "half-true":
    half_true_articles.append(article) 
  elif article['rating'] == "mostly false":
    mostly_false_articles.append(article) 
  elif article['rating'] == "pants on fire!":
    pants_on_fire_articles.append(article)

print(f"True Articles: {len(true_articles)}")
print(f"Mostly True Articles: {len(mostly_true_articles)}")
print(f"Half True Articles: {len(half_true_articles)}")
print(f"Mostly False Articles: {len(mostly_false_articles)}")
print(f"False Articles: {len(false_articles)}")
print(f"Pant on Fire Articles: {len(pants_on_fire_articles)}")

real_titles = []
for art in true_articles:
  real_titles.append(art['text'])

for art in mostly_true_articles:
  real_titles.append(art['text'])
...
false_titles = []
for art in false_articles:
  false_titles.append(art['text'])

for art in mostly_false_articles:
  false_titles.append(art['text'])

for art in pants_on_fire_articles:
  false_titles.append(art['text'])

false_titles_ls = [text for text in false_titles]
# print(alls)
false_all_words = ' '.join(false_titles)
...
dtm = vect.transform(real_titles)
repr(dtm)
...
# true-df
true_df = pd.DataFrame([x for x in (true_articles + mostly_true_articles)])
display(true_df)

# false-df
false_df = pd.DataFrame([x for x in (pants_on_fire_articles + mostly_false_articles + false_articles)])
display(false_df)
...
content = [text for text in df.text]

max_len = 0
content_len = []
for text in content:
    content_len.append(len(text.split()))
    max_len = max(len(text.split()), max_len)

print('Number of text:', len(content))
print('Max length of the text:', max_len)
print('Mean length of the text:', np.mean(content_len))

plt.figure(figsize=(20,5))
g = sns.countplot(x=content_len)
...
for pred in prediction:
  prediction_list.append(round(pred[0]))
